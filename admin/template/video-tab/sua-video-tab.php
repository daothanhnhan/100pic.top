<?php 
	...
	function video_tab ($id) {
		...
		if (isset($_POST['add_video_cat'])) {
			...
			$sql = "UPDATE video_tab SET name = '$name', link = '$link' WHERE id = $id";
			$result = mysqli_query($conn_vn, $sql);
			if ($result) {
				echo '<script type="text/javascript">alert(\'Bạn đã sửa được một loại video.\')</script>';
			} else {
	...
	$info = $action->getDetail('video_tab', 'id', $_GET['id']);
?>
