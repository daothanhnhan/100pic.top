<?php
    $news = new action_news();
    $list_news_home = $news->getListNewsNew_hasLimit(5);
?>
<div class="gb-sidebar-baivietmoinhat">
    <aside class="widget">
        <h3 class="widget-title"> bài viết mới nhất</h3>
        <div class="widget-content">
            <ul>
                <?php
                    foreach ($list_news_home as $item) {
                        $newsLang = $news->getNewsLangDetail_byId($item['news_id'], $lang);
                        $news_rows = $news->getNewsDetail_byId($item['news_id'], $lang);
                ?>
                <li>
                    <div class="item">
                        <div class="item-img">
                            <a href="/<?= $newsLang['friendly_url'] ?>">
                                <img src="/images/<?= $news_rows['news_img'] ?>" alt="" class="img-responsive">
                            </a>
                        </div>
                        <div class="item-text">
                            <h2>
                                <a href="/<?= $newsLang['friendly_url'] ?>">
                                    <?= $newsLang['lang_news_name'] ?>
                                </a>
                            </h2>
                        </div>
                    </div>
                </li>
                <?php } ?>
            </ul>
        </div>
    </aside>
</div>