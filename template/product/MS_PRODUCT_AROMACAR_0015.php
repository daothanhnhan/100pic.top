<?php                                                                        
    if (isset($_GET['slug']) && $_GET['slug'] != '') {
        ...
        $rows = $action_product->getProductList_byMultiLevel_orderProductId($rowCat['productcat_id'],'desc',$trang,45,$slug);//var_dump($rows);
    }else{
...
<script src="/src/jquery-tjgallery-2.js"></script>
<style>
    .image {
        opacity: 1;
        display: block;
...
?>
    ...
    <div class="im ">
        <img src="<?= $link_anh ?>"
            alt="Avatar" data-toggle="modal" data-target="#1modal-<?= $item['product_id'] ?>" onclick="link_to('<?= $item['friendly_url'] ?>')"  data-toggle="modal" data-target="#modal-<?= $item['product_id'] ?>" onclick="" class="image" style="width:100%">
...
<div class="pictures_tab row" style="width: 100%;">
    <?php include DIR_PAGINATION."MS_PAGINATION_AROMACAR_0001.php";?>
</div>
<?php 
    foreach ($rows['data'] as $item) { 
...
<script>
 ...
 <!-- onclick="img_sort(<?= $item['product_id'] ?>)" -->