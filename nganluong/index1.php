<?php
	...
	$anh = $action->getDetail('video', 'id', $id);//var_dump($anh['video_cat_id']);//die;
	$download = $anh['download'];
	$_SESSION['video_order'] = array($id, 0, $download);
	/////////////////
	$video_cat_id = $anh['video_cat_id'];
	$video_cat = $action->getDetail('video_cat', 'id', $video_cat_id);//var_dump($video_cat['price']);
	$video_cat_price = (int)$video_cat['price'];echo $video_cat_price;
	/////////////////////////
	if (isset($_COOKIE['ma_ctv'])) {
		$user = $action->getDetail('user', 'code', $_COOKIE['ma_ctv']);
		if (!empty($user)) {
			$price_ctv = $video_cat_price * 0.1;
			$price_ctv = (int)$price_ctv;//echo $price_ctv;
			$user_code = $_COOKIE['ma_ctv'];
			$price_ctv_giam = $video_cat_price * 0.05;
...
if (isset($_POST['submit'])) {
 ...
 //$ten= $_POST["txt_test"];
	...
	$order_code='NL_'.time();
	//Khai báo url trả về 
	$return_url= "https://100pic.top/nganluong/success2.php";
	// Link nut hủy đơn hàng
	$cancel_url= $_SERVER['HTTP_REFERER'];	
	$notify_url = "https://100pic.top/nganluong/success2.php";
	//Giá của cả giỏ hàng 
...
<div class="split right">
  <div class="centered" style="width: 90%">
    ...
    <h2 style="font-size: 30px;text-transform: uppercase;color: #217969;">Tải nhanh:<br>(<?= $anh['name'] ?>)</h2>
    ...
    <form name="Test"  method="post" action="" onsubmit="return check();">
		<table style="margin-right: auto;margin-left: auto;">
			...
			<?php if (empty($video_cat_price)) { ?>
			<tr><td><input name="txt_gia" id="gia_nl" type="hidden" size="28" placeholder="Số tiền quyên góp" value="40000" /></td></tr>
			<?php } else { ?>
			<tr><td><input name="txt_gia" id="gia_nl" type="hidden" size="28" placeholder="Số tiền quyên góp" value="<?= $video_cat_price ?>" /></td></tr>
			<?php } ?>		
			<tr><td style="text-align: center;"><input  type="text" name="code_sale" size="28" placeholder="Nhập Mã Giảm Giá" value="<?= $_COOKIE['ma_ctv'] ?>" style="height: 40px;text-align: center;border-radius: 12px;border: 2px solid #37eac9;width: 300px;font-style: italic;font-size: 18px;color: #37eac9;font-weight: bold;" onkeyup="code_salef(this.value)" onchange="code_salef(this.value)" /></td></tr>
			<tr>
				<td>
					<?php if (empty($video_cat_price)) { ?>
				    <button type="" id="gia_show" style="color: #fff;border-radius: 12px;background: #37eac9;padding: 5px 30px;border: 0;font-size: 20px;font-weight: bold;font-style: italic;width: 300px;"><?= $ctv_ma==1 ? '38,000' : '40,000' ?> vnđ</button>
					<?php } else { ?>
					<button type="" id="gia_show" style="color: #fff;border-radius: 12px;background: #37eac9;padding: 5px 30px;border: 0;font-size: 20px;font-weight: bold;font-style: italic;width: 300px;"><?= $ctv_ma==1 ? number_format($video_cat_price*0.95) : number_format($video_cat_price) ?> vnđ</button>
					<?php } ?>
				</td>
			</tr>
			...
			<tr><td><input name="gia_goc" id="gia_goc" type="hidden" size="28" placeholder="Số tiền quyên góp" value="<?= empty($video_cat_price) ? 40000 : $video_cat_price ?>" /></td></tr>
		</table>
	</form>
  </div>
</div>
