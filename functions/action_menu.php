...
class action_menu extends library{
    ...
    /*-----------------------------*/
    public function showMenu_byMultiLevel_mainMenuNoiThat($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
        ...
        if ($cate_child) {    
            ...
            foreach ($cate_child as $key => $item) {
                ...
                echo '<li class="has-sub">';
                echo '<a href="'.$url.'">'.$row[$this->nameColTitle_menuLanguage].'</a>';
                    $this->showMenu_byMultiLevel_mainMenuProduct($list_menu,$item[$this->nameColID_menu],$lang, $level+1);
                echo '</li>';            
            }
            echo '</ul>';        
        } 
    ...
    /*-----------------------------*/
    public function showMenu_byMultiLevel_mainMenuProduct($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
        ...
        if ($cate_child) {    
            echo '<ul class="row gb-mega-menu">';            
            foreach ($cate_child as $key => $item) {
                $row = $this->getMenuLanguageDetail_byId($item[$this->nameColID_menu],$lang);
                $name = $row[$this->nameColTitle_menuLanguage];
                $url = $this->setUrlFriendly_byType($row[$this->nameColIdMenu_menuLanguage],$lang);                
                ...
                echo '<div class="box_image_sub_menu_noithat"><a href="'.$url.'">'.$row[$this->nameColTitle_menuLanguage].'</a></div>';
                echo '<div class="box_item_menu_noithat">';
                    $this->showMenu_byMultiLevel_mainMenuProduct_1($list_menu,$item[$this->nameColID_menu],$lang, $level+1);
                echo '</div>';
                echo '</li>';            
            }
            echo '</ul>';        
    ...
    /*-----------------------------*/
    public function showMenu_byMultiLevel_mainMenuProduct_1($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
    ...
    public function showMenu_byMultiLevel_mainMenuNoiThat_m($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
