...
public function getProductList_byMultiLevel_orderProductId_filter($idProductCat,$order_productId,$trang, $limit, $page){
    ...
    if ($date != 0) {
        ...
        $now = strtotime($now);
        if ($date == 1) {
            $ngay = date('Y-m-d', strtotime("-30 days", $now));
        }
        if ($date == 2) {
            $ngay = date('Y-m-d', strtotime("-3 months", $now));
        }
        if ($date == 3) {
            $ngay = date('Y-m-d', strtotime("-1 years", $now));
        }
        // echo $ngay;
        $whereClause .= " AND product_date >= '$ngay'";
    }

   $whereClause .= " AND product_new = $free";

   if (isset($_GET['size'])) {
        $size = $_GET['size'];
        $whereClause .= " AND kich_co_id = $size";
   }
...
public function getProductList_byMultiLevel_orderProductId_search($idProductCat,$order_productId,$trang, $limit, $page){
    ...
    $whereClause .= ") AND product_name LIKE '%$q%'";
    // echo $whereClause;
    // echo $list_id;
    ...
    if ($idProductCat != "" && $nameColIdProductCat != "") {
		...
		$q1 = explode(" ", $q);
		foreach ($q1 as $iq1) {
			$where .= " AND product_name LIKE '%$iq1%'";
		}
    }

    $cond = "&query=".$q;
    if (isset($_GET['procat'])) {
        $cond .= "&procat=".$_GET['procat'];
    }
