...
class library{
	...
	/*------------- Hàm select list ----------------*/
	    ...
	    public function getList_user_code($table, $name_idCat, $id_Cat, $order_name, $order_state, $trang, $limit, $page, $slug = ''){
	    ...
	    public function getList_code_sale($table, $name_idCat, $id_Cat, $order_name, $order_state, $trang, $limit, $page, $slug = ''){
...
//            $results = $stmt->get_result();
	...
	/*--------------- Danh mục đệ quy ---------------*/
	        	...
	        	foreach ($cate_child as $key => $item) {
	        		...
	        		echo '<tr>';
		                ...
		                echo '<td>';
		                	...
		                	if ($_GET['page'] == 'danh-muc-tin-tuc') {
		                		echo '<a href="index.php?page=xoa-danh-muc-tin-tuc&id='.$item[$name_id].'" onclick="return confirm(\'Bạn có chắc muốn xóa.\')">Xóa</a>';
		                	}
		                	
		                	if ($_GET['page'] == 'danh-muc-san-pham') {
		                		echo '<a href="index.php?page=xoa-danh-muc-san-pham&id='.$item[$name_id].'" onclick="return confirm(\'Bạn có chắc muốn xóa.\')">Xóa</a>';
		                	}
		...
		function getTypePage_byUrl($urlFriendly,$lang){
		    ...
		    $where = "where friendly_url = '".$urlFriendly."'";
		    $sql = "SELECT * from $table $where";
		    $result = mysqli_query($conn_vn,$sql);
		    $total_record = mysqli_num_rows($result);
		    $type = $type + $total_record;		   
		...
		public function getList_video($table, $name_idCat, $id_Cat, $order_name, $order_state, $trang, $limit, $page, $slug = ''){
	        ...
	        if ($id_Cat != "" && $name_idCat != "") {
	            ...
	            if (isset($_SESSION['cat'])) {
	            	$cat = $_SESSION['cat'];
	            	$where .= " AND video_cat_id = $cat";
	            }

	            if (isset($_SESSION['app'])) {
	            	$app = $_SESSION['app'];
	            	$where .= " AND video_app_id = $app";
	            }
	            ...
	            if (isset($_GET['search']) && $_GET['search'] != '-') {
	            	$q = $_GET['search'];
	            	$q3 = str_replace("-", " ", $q);
	            	$where .= " AND (name LIKE '%$q3%' OR link LIKE '%$q%')";
	            }
	        ...
	        $total_record = mysqli_num_rows($result);//echo $total_record;
	        ...
	        if ($trang != "" && $limit != "") {
	            ...
	            $config = array(
	                ...
	                'link_full'=> $slug != '' ? '/'.$slug.'/{trang}'.$q1 : 'index.php?page='.$page.$cond.'&trang={trang}',

	                'link_first'=> $slug != '' ? '/'.$slug.'/1'.$q2 : 'index.php?page='.$page,
	            ...
	            $rows1 = array("data"=>$rows, "paging"=>$p, "tong" => $total_record);
